...
class Quarto(object):
    ...
    def run(self, printing: bool = True) -> int:
        ...
        while winner < 0 and not self.check_finished():
            ...
            piece_ok = False
            #print(f"player number {self._current_player} choosing piece")
            while not piece_ok:
            ...
            if printing: self.print()
            #print(f"player number {self._current_player} placing piece")
            while not piece_ok:
...
# QUARTO SUBCLUSS TO IMPLEMENT CUSTOM METHODS
